            ...
            try:
                ...
                for line in info_files.split('\n'):
                    if "Entry point:" in line:
                        ...
                        start = (entry_point // 0x100000) * 0x100000
                        end = start + 0x100000
...
class EntryBreakpoint(gdb.Breakpoint):
    ...
    def stop(self):
        ...
        bp_value = int(frame.read_register("rbp"))

        # return address is stored at [rbp+8]
        self.monitor.return_addr_location = bp_value + 8

        self.monitor.original_return_addr = int(
            gdb.parse_and_eval(f"*(unsigned long*)({self.monitor.return_addr_location})"))
        ...
        gdb.execute(f"x/8gx {self.monitor.return_addr_location - 24}")
...
class ReturnAddressWatchpoint(gdb.Breakpoint):
    ...
    def stop(self):
        ...
        current_return_addr = int(gdb.parse_and_eval(f"*(unsigned long*)({self.monitor.return_addr_location})"))
...
class CheckRetCommand(gdb.Command):
    ...
    def invoke(self, arg, from_tty):
        ...
        ret_addr_loc = bp_value + 8

        try:
            ret_addr = int(gdb.parse_and_eval(f"*(unsigned long*)({ret_addr_loc})"))
            print(f"[*] Current return address: 0x{ret_addr:x}")
            print(f"[*] Return address location: 0x{ret_addr_loc:x}")

            print("[*] Stack memory around return address:")
            gdb.execute(f"x/8gx {ret_addr_loc - 24}")
...
print("[*] 64-bit return address monitoring script loaded")
print("[*] Usage: monitor-ret FUNCTION_NAME")
